...
void Widget::drawCube(const QVector3D &position, float angle)
{
    ...
    cubeProgram.setUniformValue("light.position",  camera.position());
    cubeProgram.setUniformValue("light.direction", camera.front());
    cubeProgram.setUniformValue("light.cutOff",   qCos(qDegreesToRadians(12.5f)));
    cubeProgram.setUniformValue("light.outerCutOff", qCos(qDegreesToRadians(17.5f)));
