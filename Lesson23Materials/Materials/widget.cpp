...
void Widget::drawCube()
{
    ...
    cubeProgram.setUniformValue("material.diffuse",  1.0f, 0.5f, 0.31f);
    cubeProgram.setUniformValue("material.specular", 0.5f, 0.5f, 0.5f);
    ...
    const float t = qSin(qDegreesToRadians(timer.elapsed() * 0.05f));
    const QVector3D lightColor = {t * 2.0f, t * 0.7f, t * 1.3f};
    const QVector3D diffuseColor = lightColor * QVector3D(0.5f, 0.5f, 0.5f);
    const QVector3D ambientColor  = diffuseColor  * QVector3D(0.2f, 0.2f, 0.2f);

    cubeProgram.setUniformValue("light.position", lightPos);
    cubeProgram.setUniformValue("light.ambient", ambientColor);
    cubeProgram.setUniformValue("light.diffuse", diffuseColor); // darken the light a bit to fit the scene
